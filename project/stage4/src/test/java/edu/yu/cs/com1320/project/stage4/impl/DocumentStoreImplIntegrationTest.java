...
public class DocumentStoreImplIntegrationTest  {
	...
	void IntegrationTest () throws IOException, URISyntaxException {
		...
		this.setMaxDocumentBytes(176); //Docs that should now be deleted: 5, 7, 8
		assertEquals(null, documentStore.getDocument(uri5)); //Doc 5
		assertEquals (new ArrayList<>(), this.search("farmhouse")); //Doc 6
		assertEquals(new HashSet<>(), this.deleteAllWithPrefix("fr")); //Doc 7
		assertThrows(IllegalStateException.class, () -> documentStore.undo(uri8)); //Doc 8

		//8 (9,1,2,3,4; bytes = 153)
