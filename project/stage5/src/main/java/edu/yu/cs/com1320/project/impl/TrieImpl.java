...
//------------------Setter(s)-------------------//
	...
	public Set<Value> deleteAllWithPrefix(String prefix) {
		//Empty string should return an empty set. Also, if the prefix node does not exist in the trie at all, i.e. it is null, then also return empty set.
		if (prefix.equals("")) {
			print ("given prefix of no letters");
		...
		if (this.get(this.root, prefix, 0) == null) {
			print ("given prefix has a null node");
			return new HashSet<Value>();			
		}
		...
		print ("prefixAncestorKey: " + prefixAncestorKey + ". prefixAncestor:" + prefixAncestor);

		print ("Testing for prefix:" + prefix);

		if (prefixAncestor == null) {
			print ("prefixAncestor is null!");
			return new HashSet<Value>();
		}
...
//------------------Private-------------------//
    ...
    private Node get(Node x, String key, int d) {
    	print ("checking get: Node x, String key, int d" + x +" "+ key+" " + d);
  ...
  //   public List<Value> showTrie() {
		// //Make a call to .getAllDescendantValues() with the node of the prefix as the parameter. 
		// //Take the returned Set and convert it to an ArrayList. Sort it and return it.

		// return new ArrayList<>(this.getAllDescendantValues(root, new HashSet<Value>()));		
  //   }
}
