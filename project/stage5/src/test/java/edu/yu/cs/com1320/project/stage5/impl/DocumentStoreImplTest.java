...
public class DocumentStoreImplTest  {
	...
	public void init() throws IOException, URISyntaxException {
		documentStore = new DocumentStoreImpl();
		 uri1 = new URI("http://docs/oracle.com1");
		 uri2 = new URI("http://docs/oracle.com2");
		 uri3 = new URI("http://docs/oracle.com3");
		 uri4 = new URI("http://docs/oracle.com4");
		 uri5 = new URI("http://docs/oracle.com5");
		 uri6 = new URI("http://docs/oracle.com6");
		 uri7 = new URI("http://docs/oracle.com7");
		 uri8 = new URI("http://docs/oracle.com8");
		 uri9 = new URI("http://docs/oracle.com9");
		 
		 uri10 = new URI("http://docs/oracle.com10");
		 uri11 = new URI("http://docs/oracle.com11");
		 uri12 = new URI("http://docs/oracle.com12");
		 uri13 = new URI("http://docs/oracle.com13");
		...
		doc2 = documentStore.getDocument(uri2);
		doc3 = documentStore.getDocument(uri3);
		doc4 = documentStore.getDocument(uri4);
		doc5 = documentStore.getDocument(uri5);
		doc6 = documentStore.getDocument(uri6);
		doc7 = documentStore.getDocument(uri7);
		doc8 = documentStore.getDocument(uri8);
		doc9 = documentStore.getDocument(uri9);
		doc10 = documentStore.getDocument(uri10);
		doc11 = documentStore.getDocument(uri11);
		doc12 = documentStore.getDocument(uri12);
		doc13 = documentStore.getDocument(uri13);
		baseDir = System.getProperty("user.dir");
		print (baseDir);
		orignialFileArray = Paths.get(baseDir).toFile().listFiles();
	}


	@AfterEach 
	void cleanUp  () throws IOException {
		// Path doc1Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri1) + ".json");
		// Path doc2Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri2) + ".json");
		// Path doc3Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri3) + ".json");
		// Path doc4Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri4) + ".json");
		// Path doc5Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri5) + ".json");
		// Path doc6Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri6) + ".json");
		// Path doc7Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri7) + ".json");
		// Path doc8Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri8) + ".json");
		// Path doc9Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri9) + ".json");
		// Path doc10Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri10) + ".json");
		// Path doc11Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri11) + ".json");
		// Path doc12Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri12) + ".json");
		// Path doc13Path = Paths.get(baseDir + this.parseDirectoryFromURI(uri13) + ".json");


		// System.out.println(" * " + new Throwable().getStackTrace()[1] + ": " +
		// 	"Cleaning up. Status of Deletion: \n" + 
		// 	" doc1 " + Files.deleteIfExists(doc1Path) + 
		// 	". doc2 " + Files.deleteIfExists(doc2Path) + 
		// 	". doc3 " + Files.deleteIfExists(doc3Path) + 
		// 	". doc4 " + Files.deleteIfExists(doc4Path) + 
		// 	". doc5 " + Files.deleteIfExists(doc5Path) + 
		// 	". doc6 " + Files.deleteIfExists(doc6Path) +
		// 	". doc7 " + Files.deleteIfExists(doc7Path) + 
		// 	". doc8 " + Files.deleteIfExists(doc8Path) + 
		// 	". doc9 " + Files.deleteIfExists(doc9Path) + 
		// 	". doc10 " + Files.deleteIfExists(doc10Path) + 
		// 	". doc11 " + Files.deleteIfExists(doc11Path) + 
		// 	". doc12 " + Files.deleteIfExists(doc12Path) + 
		// 	". doc13 " + Files.deleteIfExists(doc13Path) 
		// 	);
		this.restoreOriginalDirectory(Paths.get(baseDir).toFile(), orignialFileArray);
	}
...
//-----------------Stage_2----------------------//
	...
	void undoLastPutDocument() {
		documentStore.undo();
		assertEquals(null,documentStore.getDocument(uri13)); //I assume that when an undo has been made, the resulting document call will give null, as a delete is just a put of null. uri9 = last put into table
	}

	@Test
	void undoPutDocumentURI() {		
		documentStore.undo(uri7);
		assertEquals(null,documentStore.getDocument(uri7)); //I assume that when an undo has been made, the resulting document call will give null, as a delete is just a put of null.
	}
...
//-----------------Stage_3----------------------//
	...
	void deleteAllWithPrefixandUndo() throws IOException, URISyntaxException {
		...
		doc1 = documentStore.getDocument(uriDoc1);
		doc2 = documentStore.getDocument(uriDoc2);
...
/* 
DEPRECATED: The tests herenin assume behavior from stage4 which does not hold in stage5
//-----------------Stage_4----------------------//
	@Test
	void S4_documentLastUseTimeUpdates() throws IOException{
	...
	@Test
	void S4_maxDocumentCountOverflowByPut () throws IOException, URISyntaxException {
	...
	@Test
	void S4_maxDocumentCountOverflowByUndo () throws IOException, URISyntaxException {
		Document doc1 = documentStore.getDocument(uri1);
	...
	@Test
	void S4_removalOverflowFromCommandSet() throws IOException{		
	...
	@Test
	void S4_multipleDocumentDeletionDocumentCount () throws IOException, URISyntaxException {
	...
	@Test 
	void S4_documentModificationNonOverflow () throws IOException, URISyntaxException {
	...
	@Test
	void S4_documentModificationOverflow () throws IOException, URISyntaxException {
	...
	@Test
	void S4_maxDocumentBytesOverflowByPutAndUndo () throws IOException, URISyntaxException {
		documentStore.setMaxDocumentBytes(2611);
...
//-----------------Stage_5----------------------//	
	@Test
	void changeDocumentStoreBaseDir() {

	}

	@Test
	void S5_documentLastUseTimeUpdates() throws IOException{
		//Put
		long initTime = doc1.getLastUseTime();
	...
	@Test
	void maxDocumentCountOverflow_DiskBehavor () throws IOException, URISyntaxException {
		//Reset - Loop with get to make sure that doc1 is now the last used document and should be moved to disk
		for (URI uri : uriList) {
			documentStore.getDocument(uri);
		}


		//In init(), 13 documents are placed, so this should push one document to disk.
		documentStore.setMaxDocumentCount(12);
		
		//At this point, the document with uri1 should have been kicked from the store due to disk to due an overflow = 13 documents in the store.		
		//Go through the DocumentStore method list and ensure proper behavior for memory overflow = document RECOVERED from disk, last used document now MOVED to disk,
		//But that the document in disk can still be accessed with get/search/delete methods.
		
		//Get:
			//Ensure that doc 2 is in memory, doc 1 in disk
			assertTrue (notinDisk(uri2) && (inDisk(uri1)));

			//Perform the get
			assertEquals (doc1, documentStore.getDocument(uri1));

			//Ensure that doc 1 is in memory, doc 2 in disk
			//assertTrue (notinDisk(uri1) && (inDisk(uri2)));
			assertTrue (notinDisk(uri1));
			assertTrue (inDisk(uri2));
		//Search:
			//Reset - Loop with get to make sure that doc1 is now the last used document and should be moved to disk
			for (URI uri : uriList) {
				documentStore.getDocument(uri);
			...
			//Perform the serach
			assertEquals (Arrays.asList(doc1), documentStore.search("Test1"));
		...
		//SearchByPrefix:
			...
			//Perform the search by prefix
			assertTrue (documentStore.searchByPrefix("Test1").contains(doc1));
		...
		//DeleteDocument:
			...
			//Perform the delete on document 1
			assertTrue (documentStore.deleteDocument(uri1));

			//Ensure that doc1 was deleted fully.
				assertEquals (null, documentStore.getDocument(uri1));
				assertEquals (new ArrayList<>(), documentStore.search("Test1"));
				assertFalse (documentStore.searchByPrefix("Test1").contains(doc1));
				assertEquals (new HashSet<>(), documentStore.deleteAll("Test1"));
				assertFalse (documentStore.deleteAllWithPrefix("Test1").contains(uri1));

			//Undo by that URI
			documentStore.undo(uri1);

			//Ensure that doc1 was restored fully.
			assertEquals (doc1, documentStore.getDocument(uri1));
			assertEquals (Arrays.asList(doc1), documentStore.search("Test1"));
			assertTrue (documentStore.searchByPrefix("Test1").contains(doc1));
		...
		//DeleteAll:
			...
			//Ensure that doc 2 is in memory, doc 1 in disk
			assertTrue (notinDisk(uri2)); 
			assertTrue ((inDisk(uri1)));	

			//Perform the delete on document 1
			assertTrue (documentStore.deleteAll("Test1").contains(uri1));
		...
		//DeleteAllWithPrefix:
			...
			//Perform the delete on document 1
			assertTrue (documentStore.deleteAllWithPrefix("Test1").contains(uri1));
	...
	void maxDocumentCountOverflow_UndoBehavor () throws IOException, URISyntaxException {
		//Test a move to disk by overflow on put:
		documentStore.setMaxDocumentCount(13);
		URI uri14 = new URI("docs/oracle.com14");
		ByteArrayInputStream content14 = new ByteArrayInputStream("Tst14".getBytes());
		documentStore.putDocument(content14, uri14, DocumentFormat.TXT);

		//Ensure that doc 14 is in memory, doc 1 in disk
		assertTrue (notinDisk(uri14) && (inDisk(uri1)));	

		//Undo by a URI of a document now in disk, doc1, which should result in the .putDocument being undone = it's deletion, and ensure that it is fully deleted from the BTree = cannot be accessed again from disk
		documentStore.undo(uri1);
			//Get
			assertEquals (null, documentStore.getDocument(uri1));
		...
		//Undo (without URI param). Make a new DS and just put this doc.
		documentStore = new DocumentStoreImpl();
		content1 = new ByteArrayInputStream("Test1".getBytes());
		documentStore.putDocument(content1, uri1, DocumentFormat.TXT);
		documentStore.setMaxDocumentCount(0);
	...
	@Test
	void S5_maxDocumentCountOverflowByUndo () throws IOException, URISyntaxException {
		//Reset - Loop with get to make sure that doc1 is now the last used document and should be moved to disk
		for (URI uri : uriList) {
			documentStore.getDocument(uri);
		}
		...
		//At this point, the document with uri1 should have been moved to disk due to an overload of 13 documents being added.
		//Ensure that doc 13 is in memory, doc 1 in disk
		assertTrue (notinDisk(uri12) && notinDisk(uri13) && (inDisk(uri1)));			
		
		//Test that undoing the earlier deletion leads to a overlod and deletion of the doc with uri2
		documentStore.undo(uri12);

		assertTrue (notinDisk(uri12) && notinDisk(uri13) && inDisk(uri1) && inDisk(uri2));			
	}

		@Test
		void S5_maxDocumentCountOverflowSetTo0 () throws IOException, URISyntaxException{
		//Undo (without URI param). Make a new DS and just put this doc. All test that a lowering of the max document count leads to deletion of the overload.
		documentStore = new DocumentStoreImpl();
		content1 = new ByteArrayInputStream("Test1".getBytes());		
		documentStore.putDocument(content1, uri1, DocumentFormat.TXT);
	...
	@Test
	void S5_removalOverflowFromCommandSet() throws IOException{		
		documentStore.deleteAllWithPrefix("");

		content1 = new ByteArrayInputStream("hb sb 3w ft e1 lj eb zl by p0 8e 4s c9 tp su 24 xd ct d1 s2 wz hd jg k6 ui 6x ic 9k z2 8y tx ao n6 mc ky 6q mz j0 jv qn oo 5x 53 8o xi ty ic 4r 31 uh 7y xt pq ya b8 co xl bv jx c4 i2 tk 36 0h k8 ec w2 6y tg v0 47 bs hn l7 86 wk mf nu uh jt 1q zr iq zi uz km 0r 6n xo ng 0r xb i4 sa ar 7n 8d o3 u5 9h kr 94 9s h5 5u 3v d5 py 0v pz uf gz em j5 p6 gg yu wu if dw kj w1 d3 41 y5 xv c3 2x 7u 2z ox 9o hc zq d1 lr q1 0i m8 aw iq kp 2t mq um d4 rv s4 20 qd uq xn o6 tq jv jb n3 cb jj ke ma qi 30 qs 1c 01 lg 01 qy fb q7 6b t5 23 m9 x4 iy k5 ro ff aj g9 1h q9 wo g6 dx 3h zy ij db wt sd 86 wh yf zq zx n3 ca hi 1k 5k ts k8 at u0 3f u3 xr x7 y5 3s 1e ut 8e vm u4 lm ld ea c4 bf ok me lw rj v7 81 7n t9 j2 v4 2m ua i6 oc n3 ux 24 94 u2 fl iw f2 g2 5u 4f cz 50 mk c3 op wu 8x to sz jy xk vm jo t2 sh 0q 37 sx mx rp c5 d0 4q og x4 0n ng i4 tt 4t 50 5q 3o ry kr gx e1 aa k3 fs k5 17 t1 ai cs um m5 l4 t3 vq gx 19 y6 qf e1 hz 3b ti t3 1c n8 yg fu jl 4o r5 2b bg 6m i1 kz n5 d7 26 p4 s2 9g vd c4 c5 w4 29 ct k1 en uh q4 cy 0z en rp on nw 08 a9 at nc lw h4 ox pe 2i 3a 9g do h7 bf d4 gc j8 bg x0 ui bz a5 vc ok bu 6y 49 pv jn 4h ek tf pp p5 rh am gh vz s1 qa j6 35 ap 0n kv y9 jf fd 7n cy dw ar 8z rp 5u a9 wa ug nn m7 f2 tg d1 4y 1f le 12 98 k4 g0 zl va dw ks lh dk t2 ry 18 2x ez 5t vg bz 2q 5s uo wl 49 gv 4g y3 31 29 rc h7 sa wt 73 wl 39 08 nb 5r pp vm mf 0p ih z3 32 cu ht as y7 79 rs mc 6d 9a 0o xl gh p6 yk h0 sk k2 t9 2m vr bh r0 yy 0p v4 1p il bh v0 yr u9 16 dd 2e yf 1t u6 6v 11 jm 4f 80 bs 6f kz ec fd z5".getBytes());		
		content2 = new ByteArrayInputStream("Test1".getBytes());
		content3 = new ByteArrayInputStream("Test3".getBytes());
		content4 = new ByteArrayInputStream("Test4".getBytes());
		content5 = new ByteArrayInputStream("Test5".getBytes());
		content6 = new ByteArrayInputStream("Test6".getBytes());
		...
		content9 = new ByteArrayInputStream("Test9".getBytes());
		content10 = new ByteArrayInputStream("It was supposed to be a dream vacation. They had planned it over a year in advance so that it would be perfect in every way. It had been what they had been looking forward to through all the turmoil and negativity around them. It had been the light at the end of both their tunnels. Now that the dream vacation was only a week away, the virus had stopped all air travel.".getBytes());
		content12 = new ByteArrayInputStream("Test2".getBytes());		
		content11 = new ByteArrayInputStream("I'm meant to be writing at this moment. What I mean is, I'm meant to be writing something else at this moment. The document I'm meant to be writing is, of course, open in another program on my computer and is patiently awaiting my attention. Yet here I am plonking down senseless sentiments in this paragraph because it's easier to do than to work on anything particularly meaningful. I am grateful for the distraction.".getBytes());
		content13 = new ByteArrayInputStream("He picked up 11 the burnt 11 end of 11 the branch 11 and made a 11 mark on 11 the stone. 11 Day 52 if 11 the 43marks on the stone were accurate. 44 He couldn't be sure. Day and nights had begun to blend together creating confusion, but he knew it was a long time. Much too long.".getBytes());
		...
		Iterator<URI> it = uriList.iterator();
		for (int i = 1; i <= 9; i++) {
			assertTrue (inDisk(it.next()));					
		}

		documentStore.setMaxDocumentBytes(1500);
		doc1 = documentStore.getDocument(uri1);

		it = uriList.iterator();
		for (int i = 2; i <= 13; i++) {
			assertTrue (inDisk(uriList.get(i-1)));					
		}		
	}

	@Test
	void S5_multipleDocumentDeletionDocumentCount () throws IOException, URISyntaxException {
		documentStore.getDocument(uri1);
		documentStore.getDocument(uri3);
		documentStore.getDocument(uri5);
		documentStore.getDocument(uri7);
		...
		assertTrue (inDisk(uri6));					
		assertTrue (inDisk(uri8));					
		assertTrue (inDisk(uri10));					
		assertTrue (inDisk(uri12));					
	}

	@Test 
	void S5_documentModificationNonOverflow () throws IOException, URISyntaxException {
		//Ensure that the modification of an already present document does not overflow the document store 
		documentStore.setMaxDocumentCount(13);
		documentStore.putDocument(new ByteArrayInputStream("Modification of a document should not overflow!".getBytes()), uri2, DocumentFormat.TXT);

		for (URI uri : uriList) {
			assertTrue(notinDisk(uri));
		}
	}

	// @Test
	// void S5_documentModificationOverflow () throws IOException, URISyntaxException {
	// 	//Ensure that a modification of an already present document does not override the docuement store unless it increases the byte count
	// 	documentStore.setMaxDocumentBytes(2611);
	// 	documentStore.putDocument(new ByteArrayInputStream("12345".getBytes()), uri7, DocumentFormat.TXT);
	...
	// 	//Ensure that doc 7 is in memory, doc 1 in disk
	// 	assertTrue (notinDisk(uri7) && (inDisk(uri1)));	
	// }

	@Test
	void S5_maxDocumentBytesOverflowByPutAndUndo () throws IOException, URISyntaxException {
		documentStore.setMaxDocumentCount(13);
		...
		documentStore.undo(uri14);
		// Now the doc with uri2 should be removed to disk
		
		//Ensure that doc 1, 14 are in memory, doc 2 in disk
		assertTrue (notinDisk(uri1) && notinDisk(uri14));
		assertTrue((inDisk(uri2)));	
	}	
