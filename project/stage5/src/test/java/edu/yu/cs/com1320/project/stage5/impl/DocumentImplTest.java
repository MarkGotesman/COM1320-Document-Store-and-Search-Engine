...
public class DocumentImplTest {
	...
	@Test
	void getAndSetLastUsedTime () {
		Document doc = new DocumentImpl(uri1, text1);
		Long updatedTime = System.nanoTime();
		doc.setLastUseTime(updatedTime);
		assertEquals(updatedTime, doc.getLastUseTime());
	}

	@Test
	void getWordMap () {
		Document doc = new DocumentImpl(uri1, text1);		
		Map<String,Integer> docWordMap= doc.getWordMap();

		//Check that the returned map is a copy and does not enable modification of the orginal data
		docWordMap.put("Lorem", 0);
		docWordMap.put("ipsum", 23);		
		assertEquals (1, doc.wordCount("Lorem"));		
		assertEquals (1, doc.wordCount("ipsum"));		
	} 

	@Test 
	void createDocumentWithWordMap() {
		//Intentionally feed a document the wrong word map upon creation, assume that the methods should function with the given word map and not the actual contents of the document
		Document doc1 = new DocumentImpl(uri1, text1);		
		Map<String,Integer> doc1WordMap = doc1.getWordMap();

		Document doc2 = new DocumentImpl(uri2, text2, doc1WordMap);

		assertEquals (1, doc2.wordCount("Lorem"));		
		assertEquals (1, doc2.wordCount("ipsum"));		

		assertEquals (0, doc2.wordCount("consectetur"));		
		assertEquals (0, doc2.wordCount("adipiscing"));	
	}

	@Test 
	void setDocumentWithWordMap() {
		//Check that document creation with null as the third param correctly produces the correct word map; also, that when changed with setwordmap, changes the word map.
		Document doc1 = new DocumentImpl(uri1, text1);		
		Map<String,Integer> doc1WordMap = doc1.getWordMap();

		Document doc2 = new DocumentImpl(uri2, text2, null);

		assertEquals (1, doc2.wordCount("consectetur"));		
		assertEquals (1, doc2.wordCount("adipiscing"));	

		doc2.setWordMap(doc1WordMap);
